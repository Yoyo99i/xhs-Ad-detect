...
function buildPrompt(noteText) {
  return [
    ...
    "",
    "## 给普通用户的提醒",
    "面向看到这份报告的普通读者。这只是 AI 的风险提示，不等于这篇笔记一定是广告——博主也可能是真实体验写得太像推广。所以语气要委婉、留有余地，像朋友善意提醒，不要用'拆穿''罪证''违规'这类定性词。",
    "要求：",
    "1. 先从笔记里挑 1-2 句最像营销的话，用引号引出，然后温和地表达'读到这里可以多想一下'（例如'「敷完立刻有效果」这种说法，未必每个人都这样，护肤效果因人而异'）；不要用'这类话术听听就好''这种效果承诺无科学依据'这种生硬定性的句子；",
    "2. 如果命中多个维度，可以用加粗短词分点（例如**只夸不贬**、**效果说得太满**），但措辞是'这几处读起来有点像推广'而非'这是广告'；",
    ...
    "4. 结尾给 1-2 条温和建议：多看几篇不同博主的测评（包括差评）、下单前想想自己是否真需要、严重时优先看医生等。不要用'勿冲动囤货''依法举报'这类强硬措辞；",
    "5. 总字数 80-180 字，可以用'多想一下''未必''也可能'等留有余地的词；",
