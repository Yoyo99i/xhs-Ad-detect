...
function buildPrompt(noteText) {
  return [
    ...
    "规则：",
    "",
    "【最高优先级铁律·必须先于一切维度判定执行】",
    "本工具测「疑似营销话术浓度」，非法律裁定。判定任何维度前，必须先应用以下两条：",
    "① 宁漏不误伤：真实分享与软广的唯一硬分野=利益关联（购买引导/品牌合作/未披露利益），而非表达形式。严禁仅因以下特征判定为营销：正向/利他表达、未讲失误过程、经验贴骨架、个人亲身经历、多品类生活记录、情绪宣泄。先找利益信号；形式信号仅在利益信号并存时才上调对应维度。存疑一律倾向「疑似浓度低」。",
    "② F2 模板不得单独命中：凡「数字对比+方法命名」（如「140➡️100斤｜21天暴瘦法」「测了30款面霜」），属高频模板 F2，但真实打卡/真实经验贴也用同样句式。F2 不得单独命中任何维度；仅当与利益信号并存时才作弱增强（低权重）。单见「数字+方法名」无利益关联→视为真实记录，不计入。",
    "",
    ...
    "2. '｜贡献：XX%' 是必填项，表示这个维度对整体风险的加权贡献百分比，计算方式：先按程度取系数（轻=0.5、中=1.0、重=1.5，与知识库评分体系一致），乘以该维度权重（推广未标明0.22、利益未披露0.18、绝对化用语0.16、隐性营销意图0.14、功效夸张0.12、信息可信度0.10、情绪裹挟0.08），所得小数乘以 100 四舍五入取整，写成整数百分比。例：绝对化用语程度=中 → 1.0×0.16=0.16 → 贡献 16%（写 16%）。",
