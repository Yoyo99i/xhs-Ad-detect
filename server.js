...
function buildPrompt(noteText) {
  return [
    ...
    "格式与可读性要求（必须遵守）：",
    "1. 用 Markdown 无序列表输出，每一点独立一行，每一点最多两句，第一句说现象，第二句给提醒；",
    "2. 禁止出现大段连续文字、禁止出现'任务已完成''以上''综上所述''本结果''本次分析''总而言之'等 AI 套话；",
    "3. 先从笔记里挑 1-2 句最像营销的话，用引号引出，然后用自己的话、结合这篇笔记的具体内容表达'这个说法值得多想想'。每篇笔记写法应不同，禁止固定模板；",
    "4. 如果命中多个维度，用加粗短词分点。每个加粗短词必须对应'评分依据'里列出的某个维度（不要 invent 新词），后面紧跟带引号的具体证据原文；",
    "5. 品类科普（必须）：如果正文涉及护肤/美妆/化妆品/医护/医美/保健品等，给一句与该品类相关的常识提醒，具体内容根据正文实际品类写，不要生搬硬套；",
    "6. 结尾给 1-2 条温和建议，必须针对这篇笔记具体情况来写，不要每次一样。不要用'勿冲动囤货''依法举报'等强硬措辞；",
    "7. 总字数 120-260 字，多用'多想一下''未必''也可能''说不定'等留有余地的词；",
    "8. 禁止写'本结果由AI生成'等免责声明（页面底部已有）。",
    "",
    "【笔记正文】",
    noteText
  ].join("\n");
